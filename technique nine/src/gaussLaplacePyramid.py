    ...
    for k in range(0, len(gauss_pyramid_list)):
        ...
        if k + 1 == len(gauss_pyramid_list):
            ...
            if expandedImage.size > gauss_pyramid_list[k].size:
                expandedImage = expandedImage[:gauss_pyramid_list[k].shape[0], :gauss_pyramid_list[k].shape[1]]
...
def blend(first_laplace_pyr_list, second_laplace_pyr_list, mask_gaussian_pyr_list ):
    ...
    for i in range(0, len(mask_gaussian_pyr_list)):
        #LS(i, j) = (1 - GR(i, j))*LA(i, j) + GR(i, j,)*LB(i, j);
        blended_layer =  (1 - mask_gaussian_pyr_list[i])*first_laplace_pyr_list[i] + mask_gaussian_pyr_list [i]*second_laplace_pyr_list[i]
...
def collapse(blended_pyramid):
    ...
    for i in range(len(blended_pyramid) - 1, 0, -1):
        ...
        if expanded_layer.size > blended_pyramid[i - 1].size:
            expanded_layer = expanded_layer[:blended_pyramid[i - 1].shape[0], :blended_pyramid[i - 1].shape[1]]   
...
def main():
    ...
    # Q2C 1. Read in the testing image 
    im = plt.imread("testpattern1024.tif")
    im = rgb2gray(im)

    # Q2C 2. Create a gaussian pyramid list from testing image with 3 level 0-3
    gaus_list = gauss_pyramid(im, 3)

    # Q2C 3. Plot the original image
    plt.figure(figsize=(14,14))
    plt.imshow(im, cmap="gray"), plt.title("Original", size=12)

    # Q2C 4. Plot all Gaussian pyramid image in the list
    plt.figure(figsize=(14,14))
    plt.subplots_adjust(left=0.05, top=0.95, right = 0.95, bottom=0, wspace=0.05, hspace=0.05)
    plt.subplot(221), plt.imshow(gaus_list[0], cmap="gray"), plt.title("Gaussian Pyramid Level 0", size=8)
    plt.subplot(222), plt.imshow(gaus_list[1], cmap="gray"), plt.title("Gaussian Pyramid Level 1", size=8)
    plt.subplot(223), plt.imshow(gaus_list[2], cmap="gray"), plt.title("Gaussian Pyramid Level 2", size=8)
    plt.subplot(224), plt.imshow(gaus_list[3], cmap="gray"), plt.title("Gaussian Pyramid Level 3", size=8)

    # Q2C 5. Create a laplace pyramid list from testing image with 3 level 0-3
    laplace_list = laplace_pyramid(gaus_list)

    # Q2C 6. Plot all Laplace pyramid image in the list
    plt.figure(figsize=(14,14))
    plt.subplots_adjust(left=0.05, top=0.95, right = 0.95, bottom=0, wspace=0.05, hspace=0.05)
    plt.subplot(221), plt.imshow(laplace_list[0], cmap="gray"), plt.title("Laplace Pyramid Level 0", size=8)
    plt.subplot(222), plt.imshow(laplace_list[1], cmap="gray"), plt.title("Laplace Pyramid Level 1", size=8)
    plt.subplot(223), plt.imshow(laplace_list[2], cmap="gray"), plt.title("Laplace Pyramid Level 2", size=8)
    plt.subplot(224), plt.imshow(laplace_list[3], cmap="gray"), plt.title("Laplace Pyramid Level 3", size=8)

    #######################################################################################################
    #GRAY SCALE - Do not Grade
    # # Q2D: 1. Read in all apple, orange, mask image and grayscale them
    ...
    # # Q2D 2. Calculate the gaussian and laplace accordingly 
    ...
    # # Q2D 3. Calculate the blended_pyramid list
    # blended_output = blend(laplace_apple, laplace_orange, gaus_mask)

    # # Q2D 4. Collapse all image in the blended_pyramid list
    # collapse_output = collapse(blended_output)

    # # Q2D 5. Plot the output image
    # plt.figure(figsize=(14,14))
